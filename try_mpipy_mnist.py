...
port_mapping=[61137,61138,61136]
...
def train(model, device, train_loader, optimizer, epoch, rank):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        # 打印每个批次的损失
        if batch_idx % 5 == 0:  # 每10个批次打印一次
            print(f'Client {rank} - Epoch {epoch}, Batch {batch_idx}, Loss: {loss.item():.6f}')
...
if rank == 0:
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print(f'Server - Epoch {epoch}, Test Loss: {test_loss:.4f}, Test Accuracy: {test_accuracy:.2f}%')
else:
    # 客户端节点负责本地训练
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        # 打印客户端的训练信息
        print(f'Client {rank} - Epoch {epoch}, Local Training Updated and Model Parameters Sent.')
